public class RecursiveMath {
	...
	public static int power(int x, int y) {
		...
		// Replace the next line with code that recursively calls power (don't use Math.pow)
		if(y < 1)
			result = 1;
		else
	...
	public static int fib(int num) {
		...
		// Replace the next line with code that recursively calls fib
		if(num == 0)
			result = 0;
		else if(num <= 2)
			result = 1;
	...
	public static int gcd(int x, int y) {
		...
		// Replace the next line with code that recursively calls gcd
		if(y == 0)
			result = x;
		else
			result = gcd(y, x % y);
